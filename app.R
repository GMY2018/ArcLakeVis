...
server <- function(input, output) {
  ...
  output$AllMeanCurves <- renderPlot({
    ...
    if(input$allmeans){
      ...
      ups   <- cmeans + cses # 1.96*(cses)
      downs <- cmeans - cses
      for (kk in 1:9){
        lines(ti, cmeans[kk,], lty=1, lwd=2, col=groupcols[kk])
        # up  <- ups[kk, ]
        # down <- downs[kk,] 
        # polygon(c(ti, rev(ti)), c(up, rev(down)), col=makeTransparent(groupcols[kk], alpha=20), 
        #         border=makeTransparent(groupcols[kk], alpha=20), lwd=2)
      }
