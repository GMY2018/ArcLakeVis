...
# load("newalldat.Rdata")  # original data
# load("newalldat2.Rdata")  # smoothed data
# load("maskbrick.Rdata")
# load("meanefd.Rdata")
...
shinyServer(function(input, output, session) {
  ...
  observe({
    ...
    isolate({
      ...
      selectedLake <<- Lake   # use <<- to make it an environmental variable
  ...
  output$GamSummary <- renderPrint({
    ...
    fitGam <- GamData()
    if (!is.null(fitGam)) {
      print('Here is the fitted model.')
      summary(fitGam)
    }
